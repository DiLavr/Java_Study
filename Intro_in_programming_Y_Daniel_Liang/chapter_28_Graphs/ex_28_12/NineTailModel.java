...
public class NineTailModel {
	  ...
	  private List<AbstractGraph.Edge> getEdges() { // get edges
	   ...
	   for (int u = 0; u < NUMBER_OF_NODES; u++) {
	    for (int k = 0; k < 9; k++) {
	     ...
	     if (node[k] == 'H') {
	      ...
	      // v
	      edges.add(new AbstractGraph.Edge(v, u)); // add an edge
	     }
	    }
	   }
	  ...
	  public static int getFlippedNode(char[] node, int position) { // flip cells
	   int row = position / 3;
	  ...
	  public static void flipACell(char[] node, int row, int column) { // flip a cell
	  ...
	  public static int getIndex(char[] node) { //get index for a node
	   int result = 0;
	  ...
	  public static char[] getNode(int index) { //get node for an index
	  ...
	  public List<Integer> getShortestPath(int nodeIndex) { //shortest path
	   return tree.getPath(nodeIndex);
	  }

	  public static void printNode(char[] node) { // display node
	   for (int i = 0; i < 9; i++)
	    if (i % 3 != 2)
